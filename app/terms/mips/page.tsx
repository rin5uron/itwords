...
10⁻⁶ 秒 = 1マイクロ秒なので、答は「1/MIPS マイクロ秒」で表せる。`}</code></pre>
          ...
          <h3>時間の単位の階段：秒 → ミリ秒 → マイクロ秒</h3>
          <p>
            時間の単位を「階段」で覚えましょう。1段降りるごとに1000分の1になります。
          </p>
          ...
          <h3>なぜマイクロ秒なのか？</h3>
          <p>
            コンピュータが1つの命令を処理するとき、次の3つのステップを踏みます。
          </p>
          <ul>
            <li>命令を受け取る（メモリから読み込む）</li>
            <li>命令を実行する（計算や処理を行う）</li>
            <li>結果を返す（メモリに書き込む）</li>
          </ul>
          <p>
            それぞれの処理時間は数ナノ秒〜数十ナノ秒程度。それらを合わせると、1命令あたり<strong>数十〜数百ナノ秒</strong>になります。
            100万命令を1秒で実行する（1 MIPS）とき、1命令あたりは 1 ÷ 100万 = <strong>1マイクロ秒（10⁻⁶秒）</strong>。
            つまり、コンピュータの「命令1つ分の処理時間」の大きさが、ちょうどマイクロ秒のオーダーになるからです。
          </p>
          <div className="key-point-box">
            <strong><i className="fas fa-lightbulb" aria-hidden /> ポイント：</strong>
            マイクロ秒は10⁻⁶秒（10のマイナス6乗）。1μs = 0.000001秒。コンピュータの命令実行時間はこの単位で表すのが自然。
          </div>
