...
| `gsc-bigquery-analyst` | Search Console（検索パフォーマンス）と BigQuery（GA4生データ）を API で自動取得して統合分析。`/seo-check` の手貼りを自動化。初回は [セットアップ](./.claude/skills/gsc-bigquery-analyst/references/setup.md) が必要 |
