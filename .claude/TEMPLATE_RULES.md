...
### 基本方針
- **点線は使わない**（`dashed` / `dotted` の border は禁止）
- **まとめ・強調は枠で整理する**（`.key-point-box` を使用）

### ここはこの枠を使う
| 用途 | クラス | 説明 |
|------|--------|------|
| ポイント・わかったこと・一言でいうと | `key-point-box` | 左ボーダー強調の枠。SSHページの「一言でいうと」と同じデザイン |
| 関連用語比較 | `comparison-table` | テーブル形式 |
| コード例 | `code-example` | ダーク背景のコードブロック |

### 禁止
- `border: dashed` / `border: dotted` の使用
- 点線による区切りや装飾
...
**最終更新**: 2026-02-07（枠デザイン・点線禁止ルール追加）
